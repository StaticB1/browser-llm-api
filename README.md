...
- A display for Chrome (runs **non-headless** on purpose — Gemini blocks headless). This setup uses `DISPLAY=:1`.
...
**Re-auth must be done in the automation's own browser.** `nodriver` launches Chrome with `--password-store=basic`; a normal Chrome uses the system keyring, and cookies written by one cannot be decrypted by the other. So do **not** sign in with a plain `google-chrome` — use the helper:
